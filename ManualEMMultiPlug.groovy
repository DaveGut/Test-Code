...
////////////////////////////////////////////////////////
metadata {
	definition (name: "Manual Kasa ${type()}",
				namespace: "davegut",
...
////////////////////////////////////////
	if (!getDataValue("type")) {
		...
		message += "\n\t\t\t<b>Application set a manual installation.</b>  System is locked to LAN comms."
	} else if (parent.useKasaCloud) {
...
////////////////////////////////////////////////////////////////////	
	if (!getDataValue("type")) {
		def resp = setDeviceIp()
		if (resp == "A preference is not set.") {
			logWarn("Updated: ${resp}. Exiting method updated.")
			return
		}
		logInfo ("Updated: ${resp}")
		pauseExecution(1000)
	}
...
def setDeviceIp() {
	...
	if (!device_IP || !plug_No) {
		return "A preference is not set."
	}
	if (getDataValue("deviceIP") != device_IP.trim()) {
		updateDataValue("deviceIP", device_IP.trim())
		message += "\n\t\t\tDevice IP set to ${device_IP.trim()}."
	}
	if (!getDataValue("plugId")) {
		updateDataValue("plugNo", plug_No)
		message += "\n\t\t\tPlug Number set to ${plug_No}."
		message += "\n\t\t\tGetting device data."
		sendLanCmd("""{"system":{"get_sysinfo":{}}}""", "setDeviceData")
	}
	return message
}
/////////////////////////////////////////////////////////////////
def setDeviceData(message) {
	def resp = parseLanMessage(message)
	if (resp.type != "LAN_TYPE_UDPCLIENT") {
		logWarn("setDeviceData: Device unreachable.  Manual installation not available.")
		return
	}
...
def sendLanCmd(command, action = "parse") {
	logDebug("sendLanCmd: command = ${command}, action = ${action}")
...
def setBindUnbind(cmdResp) {
	if (cmdResp.get_info) {
		...
		if (type().contains("Multi") && getDataValue("type")) {
