...
01.13.19	4.0.04. Various enhancements:
			...
			b.	Add 10 minute timeout for trace logging. Will
				...
				installation.
SPECIAL TEST VERSION OF DRIVER.
...
def updated() {
	...
	if (getDataValue("deviceIP")) { 
		...
		setCurrentDate()
		runIn(5, getEnergyData)
	}
}

def getEnergyData(){
	getEnergyStats(state.yearToday)
}
...
def powerResponse(response) {
	...
	log.info "${device.label}: Power is ${power} Watts."
	getEnergyThisMonth()
}

def getEnergyThisMonth(){
	logTrace("getConsumption: month = ${state.monthToday} / year = ${state.yearToday}")
	sendCmd("""{"emeter":{"get_daystat":{"month": ${state.monthToday}, "year": ${state.yearToday}}}}""", "energyThisMonthResponse")
}
...
def energyThisMonthResponse(response) {
	...
	for (int i = 0; i < dayList.size(); i++) {
		energyData = dayList[i]
		if(energyData.day == state.dayToday) {
			energy = energyData."${state.energyScale}"
 		}
	}
	if (state.energyScale == "energy_wh") { energy = energy/1000 }
	
	state.energyThisMonth = dayList
	sendEvent(name: "energy", value: energy)
	logTrace("${device.label}: Energy Today is ${energy} kWh")
}

def getEnergyStats(year) {
