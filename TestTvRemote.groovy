...
3.1	- Updated ON command to use one of three user-selectable methods.
===========================================================================================*/
...
def on() {
	...
	if (wolMethod == "2") {
		def wolMac = device.deviceNetworkId.replaceAll("-","").replaceAll(":","").replaceAll(" ","")
		...
		def wol = new hubitat.device.HubAction(cmd,
		...
		def wol = new hubitat.device.HubAction("wake on lan ${getDataValue("alternateWolMac")}",
											   hubitat.device.Protocol.LAN,
											   null)
		sendHubCommand(wol)
	} else {
