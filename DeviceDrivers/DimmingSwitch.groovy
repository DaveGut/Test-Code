...
metadata {
	...
	preferences {
		...
		input ("gentleOn", "number",
			   title: "Gentle On (milliseconds)",
			   defaultValue:5000)
		input ("gentleOff", "number",
			   title: "Gentle Off (milliseconds)",
			   defaultValue:5000)
		input ("fadeOn", "number",
			   title: "Fade On (milliseconds)",
			   defaultValue:5000)
		input ("fadeOff", "number",
			   title: "Fade Off (milliseconds)",
			   defaultValue:5000)
...
def installed() {
	...
	pauseExecution(3000)
	sendCmd("""{"smartlife.iot.dimmer":{"get_dimmer_parameters":{}, "get_default_behavior":{}}}""")
...
def setLevel(percentage, transition = null) {
	if (transition == null) { transition = fadeOn }
	else if (transition == 0) { transiton = 50 }
	else { transition = transition * 1000 }
	percentage = percentage.toInteger()
	if (percentage < 0) { percentage = 0 }
	if (percentage > 100) { percentage = 100 }
	sendCmd("""{"smartlife.iot.dimmer":{"set_dimmer_transition":{"brightness":${percentage},""" +
			""""duration":${transition}}}}""")
	def updates = [:]
	updates << [switch: "on"]
	sendEvent(name: "switch", value: "on", type: "digital")
	updates << [level: percentage]
	sendEvent(name: "level", value: percentage, type: "digital")
	logInfo("setLevel: ${updates}")
	def pollTime = 2 + (transition/1000).toInteger()
	runIn(pollTime, poll)
}
...
def setDimmerConfig(response) {
	...
	if (response["get_dimmer_parameters"]) {
		...
		device.updateSetting("fadeOnTime", [type:"integer", value: params.fadeOnTime])
		device.updateSetting("fadeOffTime", [type:"integer", value: params.fadeOffTime])
		device.updateSetting("gentleOnTime", [type:"integer", value: params.gentleOnTime])
		device.updateSetting("gentleOffTime", [type:"integer", value: params.gentleOffTime])
	}
