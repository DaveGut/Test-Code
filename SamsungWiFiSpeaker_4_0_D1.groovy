...
def refresh() {
	...
	if (isConnected == "yes") {
		if (state.playingNotification == false) {
			...
			def resp = sendSyncCmd("/UIC?cmd=%3Cname%3EGetFunc%3C/name%3E")
			resp = getVolume()
			resp = getPlayStatus()
			resp = sendSyncCmd("/UIC?cmd=%3Cname%3EGetAcmMode%3C/name%3E")
			setTrackDescription()
		} else {
			logData << [status: "notUpdated", reason: "playing Notification"]
		}
	} else {
		logWarn("refresh: device is not connected to the LAN")
		logData << [status: "notUpdated", reason: "not Connected to LAN"]
	}
	logDebug("refresh: ${logData}")
logTrace("refresh: ${logData}")
	return isConnected
}
def lanCheck() {
	def resp = sendSyncCmd("/UIC?cmd=%3Cname%3EGetFunc%3C/name%3E")
	...
	if (isConnected != device.currentValue("isConnected")) {
		...
		clearQueue()
		logInfo("checkConnected: isConnected: ${isConnected}")
	}
	return isConnected
}
