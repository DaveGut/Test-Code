...
def parse(message) { // library marker davegut.kasaCommunications, line 120
	...
	if (message.length() > 8 && message.substring(0,4) == "0000") { // library marker davegut.kasaCommunications, line 122
		...
		if (message.length() == respLength) { // library marker davegut.kasaCommunications, line 125
			...
			state.response = message // library marker davegut.kasaCommunications, line 128
			state.respLength = respLength // library marker davegut.kasaCommunications, line 129
		} // library marker davegut.kasaCommunications, line 130
	} else if (message.length() == 0 || message == null) { // library marker davegut.kasaCommunications, line 131
		return // library marker davegut.kasaCommunications, line 132
	} else { // library marker davegut.kasaCommunications, line 133
		def resp = state.response // library marker davegut.kasaCommunications, line 134
		resp = resp.concat(message) // library marker davegut.kasaCommunications, line 135
		if (resp.length() == state.respLength) { // library marker davegut.kasaCommunications, line 136
			state.response = "" // library marker davegut.kasaCommunications, line 137
			state.respLength = 0 // library marker davegut.kasaCommunications, line 138
			extractResp(message) // library marker davegut.kasaCommunications, line 139
		} else { // library marker davegut.kasaCommunications, line 140
			state.response = resp // library marker davegut.kasaCommunications, line 141
...
def extractResp(message) { // library marker davegut.kasaCommunications, line 146
	if (message.length() == null) { // library marker davegut.kasaCommunications, line 147
		logDebug("extractResp: null return rejected.") // library marker davegut.kasaCommunications, line 148
		return  // library marker davegut.kasaCommunications, line 149
	} // library marker davegut.kasaCommunications, line 150
	logDebug("extractResp: ${message}") // library marker davegut.kasaCommunications, line 151
	try { // library marker davegut.kasaCommunications, line 152
		distResp(parseJson(inputXorTcp(message))) // library marker davegut.kasaCommunications, line 153
	} catch (e) { // library marker davegut.kasaCommunications, line 154
		logWarn("extractResp: Invalid or incomplete return.\nerror = ${e}") // library marker davegut.kasaCommunications, line 155
		handleCommsError() // library marker davegut.kasaCommunications, line 156
	} // library marker davegut.kasaCommunications, line 157
} // library marker davegut.kasaCommunications, line 158

def handleCommsError() { // library marker davegut.kasaCommunications, line 160
	def count = state.errorCount + 1 // library marker davegut.kasaCommunications, line 161
	state.errorCount = count // library marker davegut.kasaCommunications, line 162
	def message = "handleCommsError: Count: ${count}." // library marker davegut.kasaCommunications, line 163
	if (count <= 3) { // library marker davegut.kasaCommunications, line 164
		message += "\n\t\t\t Retransmitting command, try = ${count}" // library marker davegut.kasaCommunications, line 165
		runIn(1, sendCmd, [data: state.lastCommand]) // library marker davegut.kasaCommunications, line 166
	} else if (count == 4) { // library marker davegut.kasaCommunications, line 167
		setCommsError() // library marker davegut.kasaCommunications, line 168
		message += "\n\t\t\t Setting Comms Error." // library marker davegut.kasaCommunications, line 169
	} // library marker davegut.kasaCommunications, line 170
	logDebug(message) // library marker davegut.kasaCommunications, line 171
} // library marker davegut.kasaCommunications, line 172

def setCommsError() { // library marker davegut.kasaCommunications, line 174
	def message = "setCommsError: Four consecutive errors.  Setting commsError to true." // library marker davegut.kasaCommunications, line 175
	if (device.currentValue("commsError") == "false") { // library marker davegut.kasaCommunications, line 176
		sendEvent(name: "commsError", value: "true") // library marker davegut.kasaCommunications, line 177
//	handle AltLAN connection // library marker davegut.kasaCommunications, line 178
		message += "\n\t\tFix attempt ${parent.fixConnection(device.currentValue("connection"))}" // library marker davegut.kasaCommunications, line 179
		logWarn message // library marker davegut.kasaCommunications, line 180
	} // library marker davegut.kasaCommunications, line 181
} // library marker davegut.kasaCommunications, line 182

def resetCommsError() { // library marker davegut.kasaCommunications, line 184
	unschedule(handleCommsError) // library marker davegut.kasaCommunications, line 185
	sendEvent(name: "commsError", value: "false") // library marker davegut.kasaCommunications, line 186
	state.errorCount = 0 // library marker davegut.kasaCommunications, line 187
} // library marker davegut.kasaCommunications, line 188

private outputXOR(command) { // library marker davegut.kasaCommunications, line 190
	def str = "" // library marker davegut.kasaCommunications, line 191
	def encrCmd = "" // library marker davegut.kasaCommunications, line 192
 	def key = 0xAB // library marker davegut.kasaCommunications, line 193
	for (int i = 0; i < command.length(); i++) { // library marker davegut.kasaCommunications, line 194
		str = (command.charAt(i) as byte) ^ key // library marker davegut.kasaCommunications, line 195
		key = str // library marker davegut.kasaCommunications, line 196
		encrCmd += Integer.toHexString(str) // library marker davegut.kasaCommunications, line 197
	} // library marker davegut.kasaCommunications, line 198
   	return encrCmd // library marker davegut.kasaCommunications, line 199
} // library marker davegut.kasaCommunications, line 200

private inputXOR(encrResponse) { // library marker davegut.kasaCommunications, line 202
	String[] strBytes = encrResponse.split("(?<=\\G.{2})") // library marker davegut.kasaCommunications, line 203
	def cmdResponse = "" // library marker davegut.kasaCommunications, line 204
	def key = 0xAB // library marker davegut.kasaCommunications, line 205
	def nextKey // library marker davegut.kasaCommunications, line 206
	byte[] XORtemp // library marker davegut.kasaCommunications, line 207
	for(int i = 0; i < strBytes.length; i++) { // library marker davegut.kasaCommunications, line 208
		nextKey = (byte)Integer.parseInt(strBytes[i], 16)	// could be negative // library marker davegut.kasaCommunications, line 209
		XORtemp = nextKey ^ key // library marker davegut.kasaCommunications, line 210
		key = nextKey // library marker davegut.kasaCommunications, line 211
		cmdResponse += new String(XORtemp) // library marker davegut.kasaCommunications, line 212
	} // library marker davegut.kasaCommunications, line 213
	return cmdResponse // library marker davegut.kasaCommunications, line 214
} // library marker davegut.kasaCommunications, line 215

//////////////////////////////////////////////////////////// // library marker davegut.kasaCommunications, line 217
private outputXorTcp(command) { // library marker davegut.kasaCommunications, line 218
	def str = "" // library marker davegut.kasaCommunications, line 219
	def encrCmd = "000000" + Integer.toHexString(command.length())  // library marker davegut.kasaCommunications, line 220
 	def key = 0xAB // library marker davegut.kasaCommunications, line 221
	for (int i = 0; i < command.length(); i++) { // library marker davegut.kasaCommunications, line 222
		str = (command.charAt(i) as byte) ^ key // library marker davegut.kasaCommunications, line 223
		key = str // library marker davegut.kasaCommunications, line 224
		encrCmd += Integer.toHexString(str) // library marker davegut.kasaCommunications, line 225
	} // library marker davegut.kasaCommunications, line 226
   	return encrCmd // library marker davegut.kasaCommunications, line 227
} // library marker davegut.kasaCommunications, line 228
////////////////////////////////// // library marker davegut.kasaCommunications, line 229
private inputXorTcp(resp) { // library marker davegut.kasaCommunications, line 230
	String[] strBytes = resp.substring(8).split("(?<=\\G.{2})") // library marker davegut.kasaCommunications, line 231
	def cmdResponse = "" // library marker davegut.kasaCommunications, line 232
	def key = 0xAB // library marker davegut.kasaCommunications, line 233
	def nextKey // library marker davegut.kasaCommunications, line 234
	byte[] XORtemp // library marker davegut.kasaCommunications, line 235
	for(int i = 0; i < strBytes.length; i++) { // library marker davegut.kasaCommunications, line 236
		nextKey = (byte)Integer.parseInt(strBytes[i], 16)	// could be negative // library marker davegut.kasaCommunications, line 237
		XORtemp = nextKey ^ key // library marker davegut.kasaCommunications, line 238
		key = nextKey // library marker davegut.kasaCommunications, line 239
		cmdResponse += new String(XORtemp) // library marker davegut.kasaCommunications, line 240
	} // library marker davegut.kasaCommunications, line 241
	return cmdResponse // library marker davegut.kasaCommunications, line 242
} // library marker davegut.kasaCommunications, line 243

def logTrace(msg){ // library marker davegut.kasaCommunications, line 245
	log.trace "[${type()} / ${driverVer()} / ${device.label}]| ${msg}" // library marker davegut.kasaCommunications, line 246
} // library marker davegut.kasaCommunications, line 247

def logInfo(msg) { // library marker davegut.kasaCommunications, line 249
	if (descriptionText == true) {  // library marker davegut.kasaCommunications, line 250
		log.info "[${type()} / ${driverVer()} / ${device.label}]| ${msg}" // library marker davegut.kasaCommunications, line 251
	} // library marker davegut.kasaCommunications, line 252
} // library marker davegut.kasaCommunications, line 253

def logDebug(msg){ // library marker davegut.kasaCommunications, line 255
	if(debug == true) { // library marker davegut.kasaCommunications, line 256
		log.debug "[${type()} / ${driverVer()} / ${device.label}]| ${msg}" // library marker davegut.kasaCommunications, line 257
	} // library marker davegut.kasaCommunications, line 258
} // library marker davegut.kasaCommunications, line 259

def debugOff() { // library marker davegut.kasaCommunications, line 261
	device.updateSetting("debug", [type:"bool", value: false]) // library marker davegut.kasaCommunications, line 262
	logInfo("debugLogOff: Debug logging is off.") // library marker davegut.kasaCommunications, line 263
} // library marker davegut.kasaCommunications, line 264

def logWarn(msg){ // library marker davegut.kasaCommunications, line 266
	log.warn "[${type()} / ${driverVer()} / ${device.label}]| ${msg}" // library marker davegut.kasaCommunications, line 267
