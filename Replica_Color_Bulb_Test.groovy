...
public static String driverVer() { return "0.1.3TEST" }
...
def setColorValue(color, internal = false) {
	Map logData = [:]
	if (color != null) {
		testColorParse(color)
...
def testColorParse(colorData) {
	log.trace colorData
	try {
		colorData = parseJson(colorData)
		log.trace "parseJson: ${colorData}, ${colorData.hue}"
	} catch (e) {log.warn "parseJson: $e"}
	try {
		colorData = new JSONObject(colorData)
		log.trace "JSONObject: ${colorData}, ${colorData.hue}"
	} catch (e) { log.warn "JSONObject: $e"}
}
