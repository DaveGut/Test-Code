...
def getDeviceStatus() {
	...
	try {
		def waterLevel = resp["samsungce.washerWaterLevel"].waterLevel.value
		logData << [waterLevel: setEvent("waterLevel", waterLevel)]
	} catch (error) {
		logWarn("waterLevel: ${error}")
	}
	
	try {
		def waterTemperature = resp["custom.washerWaterTemperature"].washerWaterTemperature.value
		logData << [waterTemperature: setEvent("waterTemperature", waterTemperature)]
	} catch (error) {
		logWarn("waterTemperature: ${error}")
	}
	
	try {
		def spinLevel = resp["custom.washerSpinLevel"]washerSpinLevel.value
		logData << [spinLevel: setEvent("spinLevel", spinLevel)]
	} catch (error) {
		logWarn("spinLevel: ${error}")
	}
	
	try {
		def bubbleSoak = resp["samsungce.washerBubbleSoak"].value
		logData << [bubbleSoakl: setEvent("bubbleSoak", bubbleSoak)]
	} catch (error) {
		logWarn("bubbleSoak: ${error}")
	}
	...
	try {
		def remainingTime = resp["samsungce.washerDelayEnd"].remainingTime.value
	...
	try {
		def machineState = resp["washerOperatingState"].machineState.value
	...
	try {
		def jobState = resp["washerOperatingState"].washerJobState.value
...
def setMachineState(machState) {
	def respData = sendCommand("main", "washerOperatingState", "setMachineState", [machState])
