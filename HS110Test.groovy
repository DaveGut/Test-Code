...
11/29	5.3.4	Added firmware version to device data section.
===================================================================================================*/
def driverVer() { return "TEST" }
metadata {
	definition (name: "TEST Kasa EM Plug",
    			namespace: "davegut",
...
def setFirmwareVersion(resp) {
	def firmwareVersion = resp.system.get_sysinfo.sw_ver
	state.remove("getFirmwareVersion")
	updateDataValue("deviceFWVer", firmwareVersion)
	logInfo("setFirmwareVersion: ${firmwareVersion}")
	setSysInfo(resp)
}
...
def distResp(response) {
	...
	def month = new Date().format("M").toInteger()
	if(state.getFirmwareVersion == true) {
		setFirmwareVersion(resp)
	} else if(resp.system) {
		setSysInfo(resp)
...
private sendCmd(command) {
	...
	if (now() - state.lastConnect > 35000 ||
	   device.name == "HS100" || device.name == "HS200" || device.name == "HS110") {
