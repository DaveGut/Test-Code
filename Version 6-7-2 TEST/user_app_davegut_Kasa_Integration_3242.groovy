...
===== 6.7.2 Changes =====
All 6.7.2 Changes are documented as code comment preceeded by "6.7.2 Change"
Application Changes
	1:	(Cleanup) Remove various items that are no longer necessary. General cleanup.
	2:	(Ease of MX) Add method nameSpace and use as namespace in code.  Add installOnOpen
	3:	(User request) Correct lanSegment persistance issue by not removing lanSegment on startup.
	4:	(Correct error) Changes to handle HS300 Message too long to parse.  When encountered in the app, adds routine to create the six plugs using a temporary name and adding altComms data to the device when adding.  Additionally, mask the out-of-bound error generated when truncated message fix fails.
Driver Change:
	A:	(Ease of MX) lib KasaCommon. Add methods nameSpace(). Modify drivers to utilize.
	B:	(Cleanup) All Drivers: Remove driver version as no longer necessary.  Remove data element driver version(avoid user confusion).  Update library Logging remove ref to driver version.
	C:	If device is HS300, use AltLan comms for get_sysinfo automatically. Rename Hubitat device to plug alias on installation. Also modifies libraries in support of these changes.
	D.	(Error) Restore error handling since parseWarning no longer returns a timeout to the calling method.
===================================================================================================*/
//	6.7.2 Change 1: Remove appVersion as no longer necessary.
//def appVersion() { return "6.7.1" }
//	6.7.2 Change 2: Add method nameSpace and use as namespace in code.
...
//	6.7.2 Change 4: Add device data of altComms = true for HS300 with truncated message on discovery.
...
def listDevices() {
	...
	if (devices == null) {
		...
		devices.each{
			...
			if (isChild) {
//	6.7.2 Change 1: Remove driverVer as no longer necessary.
...
def findDevices() {
	...
	def delay = 50 * (finish - start) + 4000
...
//	6.7.2 Change 4: Changes to handle HS300 Message too long to parse.
//	Added another else to below (else if model = HS300) (accessed only if the return message was truncated).
//	Create TEMP plug aliases that will be changed in device on installation.
	} else if (model == "HS300") {
...
//	6.7.2 Change 4. Update state.devices to new device alias.
...
//	6.7.2 Change 1: Remove appVersion as no longer necessary in all log messages
