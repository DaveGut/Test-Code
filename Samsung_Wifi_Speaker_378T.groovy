...
metadata {
	...
	preferences {
		...
		def positions = ["fl": "stereo left",
						 ...
						 "rl": "surround left",
						 "rr": "surround right"]
		input ("deviceIp", "string", title: "Update Device IP")
...
def updated() {
	...
	unschedule()
	if (!deviceIp) {
		def existingIp = getDataValue("deviceIP")
		device.updateSetting("deviceIp", [type:"string", value: existingIp])
	} else {
