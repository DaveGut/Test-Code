/*	===== HUBITAT Samsung Refrigerator Cavity CHILD
...
metadata {
	definition (name: "Samsung Refrigerator-cavity",
				namespace: "davegut",
...
def setCollingSetpoint(setpoint) {
	logDebug("setCollingSetpoint: ${setpoint}")
	def cmdData = [
		component: getDataValue("componentId"),
		capability: "thermostatCoolingSetpoint",
		command: "setCoolingSetpoint",
		arguments: [setpoint]]
	parent.cmdRespParse(syncPost(cmdData))
}
...
def statusParse(respData) {
	...
	def temperature = parseData.temperatureMeasurement.temperature.value.toInteger()
	...
	def coolingSetpoint = parseData.thermostatCoolingSetpoint.coolingSetpoint.value.toInteger()
